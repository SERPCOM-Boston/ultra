...
*/
	...
	define('UPG_PLUGIN_VERSION', '1.42');
...
function upg_template_redirect() 
{
	...
	if( ! is_feed() ) 
	{
			...
			if( is_tax( 'upg_cate' ) ) 
			{
				...
				$redirect_url = upg_get_category_page_link( $term );
...
function upg_get_category_page_link( $term ) {
	...
	if( $page_settings['main_page'] > 0 ) {
		...
		if( '' != get_option( 'permalink_structure' ) ) {
    		...
    		$link = add_query_arg( 'upg_cate', $term->slug, $link );
  		}
	}
